...
from pbim_preprocessor.sampling import MeanSamplingStrategy, LinearInterpolationSamplingStrategy
...
@click.argument("output-path", type=click.Path(writable=True, dir_okay=False, path_type=Path))
...
def assemble(path: Path, output_path: Path, start_time: datetime.datetime, end_time: datetime.datetime, resolution: int, strategy: str):
    output_path.parent.mkdir(exist_ok=True, parents=True)
    assembler = Assembler(STRATEGIES[strategy], resolution)
    with CsvWriter(output_path, CHANNELS) as writer:
        for step in assembler.assemble(path, start_time=start_time, end_time=end_time, channels=CHANNELS):
