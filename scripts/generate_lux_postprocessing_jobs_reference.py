            ...
            for frequency in [250, 1000, 1500]:
                if aggregation == "nosampling":
                    input_path_parameter = (
                        f"/data/LUX/{scenario}/filtered/{aggregation}/assembled.dat"
                    )
                    output_path_parameter = f"/data/LUX/{scenario}/post-processed/reference/{aggregation}/assembled.dat"
                    output_path = Path(
                        f"k8s/assemble_jobs/lux/post-process-jobs/reference/{scenario}/{aggregation}.yml"
                    )
                else:
                    input_path_parameter = f"/data/LUX/{scenario}/filtered/{aggregation}/{frequency}Hz/assembled.dat"
                    output_path_parameter = f"/data/LUX/{scenario}/post-processed/reference/{aggregation}/{frequency}Hz/assembled.dat"
                    output_path = Path(
                        f"k8s/assemble_jobs/lux/post-process-jobs/reference/{scenario}/{aggregation}-{frequency}.yml"
                    )
