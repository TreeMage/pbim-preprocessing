...
def render_for_all_strategies_and_aggregations(
    ...
    for strategy in ["uniform"]:
        for aggregation in ["nosampling"]:
            ...
            output_path_parameter = f"/data/PBIM/{scenario}/supervised/post-processed/{aggregation}-{strategy}/{filename}/assembled.dat"
            output_path = Path(
                f"k8s/assemble_jobs/pbim/supervised/post-processed/{scenario}/{strategy}/{week:02d}/{filename}-{aggregation}-{strategy}.yml"
            )
...
def main():
    NUM_WINDOWS_TRAINING = 250000
    NUM_WINDOWS_DAMAGED = 625000
    NUM_WINDOWS_VALIDATION = 62500
    NUM_WINDOWS_TEST_UNDAMGED = 62500
    ...
    for month in MONTHS_UNDAMAGED:
        ...
        render_for_all_strategies_and_aggregations(
            "N", NUM_WINDOWS_TRAINING, month, 2, template
        )

    # Validation data undamaged
    for month in MONTHS_UNDAMAGED:
        render_for_all_strategies_and_aggregations(
            "N", NUM_WINDOWS_VALIDATION, month, 3, template
        )
    ...
    for scenario in ["S1", "S2", "S3"]:
        render_for_all_strategies_and_aggregations(
            scenario, NUM_WINDOWS_DAMAGED, "july", 2, template
        )
        render_for_all_strategies_and_aggregations(
            scenario, NUM_WINDOWS_DAMAGED, "august", 1, template
        )
